package com.shou.john.mimicvideo.view.AutoPlayVideo;
...
//                                    Log.d("trace", i + " foundFirstVideo: " + cvh.getVideoUrl());
                                    ...
                                    if (AAH_Utils.getString(_act, cvh.getVideoUrl()) != null && new File(AAH_Utils.getString(_act, cvh.getVideoUrl())).exists()) {
                                        ((AAH_CustomViewHolder) holder).initVideoView(AAH_Utils.getString(_act, cvh.getVideoUrl()), _act);
                                    } else {
...
//                                Log.d("trace", i + " contains: " + rect_parent.contains(rect_child));
    ...
    public void startDownloadInBackground(String url) {
        ...
        if ((AAH_Utils.getString(_act, url) == null || !(new File(AAH_Utils.getString(_act, url)).exists())) && url != null && !url.equalsIgnoreCase("null")) {
    ...
    public void preDownload(List<String> urls) {
        ...
        for (int i = 0; i < urls.size(); i++) {
            if ((AAH_Utils.getString(_act, urls.get(i)) == null || !(new File(AAH_Utils.getString(_act, urls.get(i))).exists())) && urls.get(i) != null && !urls.get(i).equalsIgnoreCase("null")) {
                Intent intent = new Intent(Intent.ACTION_SYNC, null, _act, AAH_DownloadService.class);
